...
primal_variable::primal_variable(initial_parameter initial, basic_parameter basic){
				...
				for (int s = 0; s <= initial.T; s++){
					...
					for (int a = 0; a < basic.nb_arcs; a++){
						...
						for (int n = 0; n < initial.nb_nodes; n++){
							flow[i][j][k][s][a][n] = IloNumVar(basic.env, 0, IloInfinity, ILOFLOAT);

							char flowName[50];
							sprintf_s(flowName, "flow(%d,%d,%d,%d,%d,%d)", (int)i, (int)j, (int)k, (int)s, (int)a, (int)n);
	...
	for (int i = 0; i < basic.nb_firms; i++){
		...
		for (int j = 0; j < initial.nb_points; j++){
			...
			char uName[50];
			sprintf_s(uName, "x(%d,%d)", (int)i, (int)j);
			...
			for (int k = 0; k < initial.nb_points; k++){
				...
				for (int s = 0; s <= initial.T; s++){
					...
					for (int n = 0; n < initial.nb_nodes; n++){
						...
						char dName[50];
						sprintf_s(dName, "d(%d,%d,%d,%d,%d)", (int)i, (int)j, (int)k, (int)s, (int)n);
				...
				for (int d = 0; d < 3; d++){
					...
					for (int n = 0; n < initial.nb_nodes; n++){
						if (basic.sharing_option == 1) {
							re[k][i][t][d][n] = IloNumVar(basic.env, 0, IloInfinity, ILOFLOAT);
						}
						else {
							re[k][i][t][d][n] = IloNumVar(basic.env, 0, 0, ILOFLOAT);
						}

						char reName[50];
						sprintf_s(reName, "re(%d,%d,%d,%d,%d)", (int)k, (int)i, (int)t, (int)d, (int)n);
	...
	for (int i = 0; i < initial.nb_points; i++){
		...
		for (int t = 0; t <= initial.T; t++){
			...
			for (int n = 0; n < initial.nb_nodes; n++){
				...
				char z_transName[50];
				sprintf_s(z_transName, "z_trans(%d,%d,%d)", (int)i, (int)t, (int)n);
...
dual_variable::dual_variable(initial_parameter initial, basic_parameter basic){
	...
	for (int k = 0; k < basic.nb_firms; k++){
		...
		char tauName[50];
		sprintf_s(tauName, "tau(%d)", (int)k);
		tau[k].setName(tauName);
		char z_tauName[50];
		sprintf_s(z_tauName, "z_tau(%d)", (int)k);
	...
	for (int k = 0; k < basic.nb_firms; k++){
		...
		for (int i = 0; i < initial.nb_points; i++){
			...
			char betaName[50];
			sprintf_s(betaName, "beta(%d,%d)", (int)k, (int) i);
			beta[k][i].setName(betaName);
			char kappaName[50];
			sprintf_s(kappaName, "kappa(%d,%d)", (int)k, (int)i);
			kappa[k][i].setName(kappaName);
			char z_betaName[50];
			sprintf_s(z_betaName, "z_beta(%d,%d)", (int)k, (int)i);
			z_beta[k][i].setName(z_betaName);
			char z_kappaName[50];
			sprintf_s(z_kappaName, "z_kappa(%d,%d)", (int)k, (int)i);
				...
				for (int t = 0; t <= initial.T; t++){
					...
					for (int tt = 0; tt <= initial.T; tt++){
						...
						for (int s = 0; s < initial.nb_nodes; s++){
							...
							char rhoName[50];
							sprintf_s(rhoName, "rho(%d,%d,%d,%d,%d,%d)", (int)k, (int)i, (int)j, (int)t, (int)tt, (int)s);
							rho[k][i][j][t][tt][s].setName(rhoName);
							char etaRName[50];
							sprintf_s(etaRName, "yetaR(%d,%d,%d,%d,%d,%d)", (int)k, (int)i, (int)j, (int)t, (int)tt, (int)s);
							etaR[k][i][j][t][tt][s].setName(etaRName);
							char dwRName[50];
							sprintf_s(dwRName, "dwR(%d,%d,%d,%d,%d,%d)", (int)k, (int)i, (int)j, (int)t, (int)tt, (int)s);
							dwR[k][i][j][t][tt][s].setName(dwRName);
							char dwLName[50];
							sprintf_s(dwLName, "dwL(%d,%d,%d,%d,%d,%d)", (int)k, (int)i, (int)j, (int)t, (int)tt, (int)s);
							dwL[k][i][j][t][tt][s].setName(dwLName);
							char z_etaRName[50];
							sprintf_s(z_etaRName, "z_etaR(%d,%d,%d,%d,%d,%d)", (int)k, (int)i, (int)j, (int)t, (int)tt, (int)s);
							z_etaR[k][i][j][t][tt][s].setName(z_etaRName);
							char z_dwRName[50];
							sprintf_s(z_dwRName, "z_dwR(%d,%d,%d,%d,%d,%d)", (int)k, (int)i, (int)j, (int)t, (int)tt, (int)s);
							z_dwR[k][i][j][t][tt][s].setName(z_dwRName);
							char z_dwLName[50];
							sprintf_s(z_dwLName, "z_dwL(%d,%d,%d,%d,%d,%d)", (int)k, (int)i, (int)j, (int)t, (int)tt, (int)s);
		...
		for (int i = 0; i < initial.nb_points; i++){
			...
			for (int t = 0; t <= initial.T; t++){
				...
				for (int s = 0; s < initial.nb_nodes; s++){
					...
					char gammaName[50];
					sprintf_s(gammaName, "gamma(%d,%d,%d,%d)", (int)k, (int)i, (int)t, (int)s);
					gamma[k][i][t][s].setName(gammaName);
					char etaIName[50];
					sprintf_s(etaIName, "yetaI(%d,%d,%d,%d)", (int)k, (int)i, (int)t, (int)s);
					etaI[k][i][t][s].setName(etaIName);
					char dwIName[50];
					sprintf_s(dwIName, "dwI(%d,%d,%d,%d)", (int)k, (int)i, (int)t, (int)s);
					dwI[k][i][t][s].setName(dwIName);
					char dwTName[50];
					sprintf_s(dwTName, "dwT(%d,%d,%d,%d)", (int)k, (int)i, (int)t, (int)s);
					dwT[k][i][t][s].setName(dwTName);
					char z_etaIName[50];
					sprintf_s(z_etaIName, "z_etaI(%d,%d,%d,%d)", (int)k, (int)i, (int)t, (int)s);
					z_etaI[k][i][t][s].setName(z_etaIName);
					char z_dwIName[50];
					sprintf_s(z_dwIName, "z_dwI(%d,%d,%d,%d)", (int)k, (int)i, (int)t, (int)s);
					z_dwI[k][i][t][s].setName(z_dwIName);
					char z_dwTName[50];
					sprintf_s(z_dwTName, "z_dwT(%d,%d,%d,%d)", (int)k, (int)i, (int)t, (int)s);
	...
	for (int k = 0; k < basic.nb_firms; k++){
		...
		for (int t = 0; t <= initial.T; t++){
			...
			for (int s = 0; s < initial.nb_nodes; s++){
				...
				char yName[50];
				sprintf_s(yName, "y(%d,%d,%d)", (int)k, (int)t, (int)s);
			...
			for (int t = 0; t <= initial.T; t++){
				...
				for (int d = 0; d < 3; d++){
					...
					for (int s = 0; s < initial.nb_nodes; s++){
						...
						char dreName[50];
						sprintf_s(dreName, "dre(%d,%d,%d,%d,%d)", (int)k, (int)i, (int)t, (int)d, (int)s);
						dre[k][i][t][d][s].setName(dreName);
						char z_dreName[50];
						sprintf_s(z_dreName, "z_dre(%d,%d,%d,%d,%d)", (int)k, (int)i, (int)t, (int)d, (int)s);
