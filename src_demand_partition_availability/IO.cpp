...
void demand_prop(initial_parameter initial, basic_parameter& basic, string root) {
	...
	if (initial.nb_points == 2) {
		for (int t = 0; t <= initial.T; t++) {
			basic.proportion.at(t).at(0).at(1) = 1;
			basic.proportion.at(t).at(1).at(0) = 1;
		}
	}
...
void demand_assign(initial_parameter initial, basic_parameter& basic, string root) {
	...
	if (initial.nb_points == 2) {
		vector<vector<vector<double> > > demand_region_pick(145, vector<vector<double> >(6, vector<double>(initial.nb_nodes)));
				...
				for (int i = 0; i <= 6; i++) {
					for (int t = 0; t <= 144; t++) {
						...
						if (i >= 1 && t >= 1) {
							...
							demand_region_pick.at(t).at(i - 1).at(s - 1) = atof(pickvalue.c_str()) * initial.demand_level * 2;
						}
					}
				}
			}
		...
		for (int s = 0; s < initial.nb_nodes; s++) {
			for (int t = 0; t <= 144; t++) {
				basic.demand_region.at(t).at(0).at(s) = demand_region_pick.at(t).at(1).at(s) + demand_region_pick.at(t).at(3).at(s) + demand_region_pick.at(t).at(5).at(s);
				basic.demand_region.at(t).at(1).at(s) = demand_region_pick.at(t).at(0).at(s) + demand_region_pick.at(t).at(2).at(s) + demand_region_pick.at(t).at(4).at(s);
			}
		}
	}
	else if(initial.nb_points == 6) {
		for (int s = 1; s <= initial.nb_nodes; s++) {
			demand_filename = "demand_" + to_string(s) + ".csv";
			demand_file = demand_loc + demand_filename;
			ifstream demand_stream(demand_file);
	...
	// output the total
	double sum_dmd_market = 0;
	for (int i = 0; i < initial.nb_points; i++) {
		for (int t = 1; t < initial.T; t++) {
			for (int s = 0; s < initial.nb_nodes; s++) {
				sum_dmd_market += basic.demand_region.at(start + t).at(i).at(s);
			}
		}
	}

	cout << endl << endl << "Market Demand = " << sum_dmd_market << endl << endl;
...
void output(initial_parameter initial, basic_parameter basic, primal_variable primal, string root){
	...
	// cout objvalue
	filename_output << root << "/results/schedule_linear_" << basic.model_option << "_alpha_" << basic.alpha[0] << '_' << basic.alpha[1]
		<< "_option_" << basic.obj_option << "_sharing_" << basic.sharing_option << "_P_" << initial.nb_plates << "_differ_" << basic.differ_ratio << "_l_" << initial.demand_level << ".csv";
...
void output_summary(initial_parameter initial, basic_parameter basic, primal_variable primal, string root) {
	...
	// ==================== output parameter ====================
	filename_output << root << "/results/summary_linear_" << basic.model_option << "_alpha_" << basic.alpha[0] << '_' << basic.alpha[1]
		<< "_option_" << basic.obj_option << "_P_" << initial.nb_plates << "_differ_" << basic.differ_ratio << "_sharing_" << basic.sharing_option << "_l_" << initial.demand_level << ".csv";
	outresult.open(filename_output.str().c_str());
	outresult << "Input Parameter" << endl;
	outresult << "Loyal Rate,# Regions,Budget,Procurement Cost,Reposition Cost,Idle Cost,Transfer Cost,Plates,Rental Price" << endl;
	outresult << '(' << basic.alpha[0] << ';' << basic.alpha[1] << ')' << ',' << initial.nb_points << ',' << initial.budget_A << ','
...
void get_val(initial_parameter initial, basic_parameter& basic, primal_variable primal, int firm_index) {
	if (firm_index == 0) { // Firm A
		// allocation
		for (int i = 0; i < initial.nb_points; i++) {
			basic.x_val_A[i] = basic.cplex.getValue(primal.park[0][i]);
		}
		for (int i = 0; i < initial.nb_points; i++) {
			for (int t = 1; t <= initial.T; t++) {
				for (int n = 0; n < initial.nb_nodes; n++) {
					...
					for (int j = 0; j < initial.nb_points; j++) {
						if (j != i && basic.period[j][i] < t) {
							basic.flow_val_A[j][i][t - basic.period[j][i]][0][n] = basic.cplex.getValue(primal.flow[0][j][i][t - basic.period[j][i]][0][n]);
							basic.flow_val_A[j][i][t - basic.period[j][i]][1][n] = basic.cplex.getValue(primal.flow[0][j][i][t - basic.period[j][i]][1][n]);
						}
					}
					// idle
					basic.flow_val_A[i][i][t - 1][2][n] = basic.cplex.getValue(primal.flow[0][i][i][t - 1][2][n]);
				}
			}
		}
		// transfer
		for (int i = 0; i < initial.nb_points; i++) {
			for (int t = 1; t <= initial.T - 1; t++) {
				for (int s = 0; s < initial.nb_nodes; s++) {
					basic.flow_val_A[i][i][t][3][s] = basic.cplex.getValue(primal.flow[0][i][i][t][3][s]);
				}
			}
		}
		// return
		for (int n = 0; n < initial.nb_nodes; n++) {
			for (int i = 0; i < initial.nb_points; i++) {
				for (int s = 1; s <= initial.T; s++) {
					for (int d = 1; d <= 2; d++) {
						if (s - d >= 1) {
							basic.re_val_A[i][s - d][d][n] = basic.cplex.getValue(primal.re[0][i][s - d][d][n]);
						}
					}
				}
			}
		...
		for (int i = 0; i < initial.nb_points; i++) {
			for (int t = 1; t <= initial.T; t++) {
				for (int ii = 0; ii < initial.nb_points; ii++) {
					for (int s = 0; s < initial.nb_nodes; s++) {
						if (i != ii && t + basic.period[i][ii] <= initial.T) {
							basic.dmd_val_A[i][ii][t][s] = basic.cplex.getValue(primal.demandvar[0][i][ii][t][s]);
						}
					}
				}
			}
		...
		// objective value
		basic.totalcost[0] = basic.cplex.getObjValue();
	}
	else {
		
		// allocation
		for (int i = 0; i < initial.nb_points; i++) {
			basic.x_val_B[i] = basic.cplex.getValue(primal.park[1][i]);
		}
		for (int i = 0; i < initial.nb_points; i++) {
			for (int t = 1; t <= initial.T; t++) {
				for (int n = 0; n < initial.nb_nodes; n++) {
					...
					for (int j = 0; j < initial.nb_points; j++) {
						if (j != i && basic.period[j][i] < t) {
							basic.flow_val_B[j][i][t - basic.period[j][i]][0][n] = basic.cplex.getValue(primal.flow[1][j][i][t - basic.period[j][i]][0][n]);
							basic.flow_val_B[j][i][t - basic.period[j][i]][1][n] = basic.cplex.getValue(primal.flow[1][j][i][t - basic.period[j][i]][1][n]);
						}
					}
					// idle
					basic.flow_val_B[i][i][t - 1][2][n] = basic.cplex.getValue(primal.flow[1][i][i][t - 1][2][n]);
				}
			}
		}
		// transfer
		for (int i = 0; i < initial.nb_points; i++) {
			for (int t = 1; t <= initial.T - 1; t++) {
				for (int s = 0; s < initial.nb_nodes; s++) {
					basic.flow_val_B[i][i][t][3][s] = basic.cplex.getValue(primal.flow[1][i][i][t][3][s]);
				}
			}
		}
		// return
		for (int n = 0; n < initial.nb_nodes; n++) {
			for (int i = 0; i < initial.nb_points; i++) {
				for (int s = 1; s <= initial.T; s++) {
					for (int d = 1; d <= 2; d++) {
						if (s - d >= 1) {
							basic.re_val_B[i][s - d][d][n] = basic.cplex.getValue(primal.re[1][i][s - d][d][n]);
						}
					}
				}
			}
		...
		for (int i = 0; i < initial.nb_points; i++) {
			for (int t = 1; t <= initial.T; t++) {
				for (int ii = 0; ii < initial.nb_points; ii++) {
					for (int s = 0; s < initial.nb_nodes; s++) {
						if (i != ii && t + basic.period[i][ii] <= initial.T) {
							basic.dmd_val_B[i][ii][t][s] = basic.cplex.getValue(primal.demandvar[1][i][ii][t][s]);
						}
					}
				}
			}
		...
		// objective value
		basic.totalcost[1] = basic.cplex.getObjValue();
	}
}
...
void assign_historical_val(initial_parameter initial, basic_parameter& basic, primal_variable primal) {
	// allocation
	for (int i = 0; i < initial.nb_points; i++) {
		basic.x_val_B_old[i] = basic.x_val_B[i];
	}
	// flow
	for (int i = 0; i < initial.nb_points; i++) {
		for (int t = 1; t <= initial.T; t++) {
			for (int n = 0; n < initial.nb_nodes; n++) {
				...
				for (int j = 0; j < initial.nb_points; j++) {
					if (j != i && basic.period[j][i] < t) {
						basic.flow_val_B_old[j][i][t - basic.period[j][i]][0][n] = basic.flow_val_B[j][i][t - basic.period[j][i]][0][n];
						basic.flow_val_B_old[j][i][t - basic.period[j][i]][1][n] = basic.flow_val_B[j][i][t - basic.period[j][i]][1][n];
					}
				}
				// idle
				basic.flow_val_B_old[i][i][t - 1][2][n] = basic.flow_val_B[i][i][t - 1][2][n];
			}
		}
	}
	// transfer
	for (int i = 0; i < initial.nb_points; i++) {
		for (int t = 1; t <= initial.T - 1; t++) {
			for (int s = 0; s < initial.nb_nodes; s++) {
				basic.flow_val_B_old[i][i][t][3][s] = basic.flow_val_B[i][i][t][3][s];
			}
		}
	}
	// return
	for (int n = 0; n < initial.nb_nodes; n++) {
		for (int i = 0; i < initial.nb_points; i++) {
			for (int s = 1; s <= initial.T; s++) {
				for (int d = 1; d <= 2; d++) {
					if (s - d >= 1) {
						basic.re_val_B_old[i][s - d][d][n] = basic.re_val_B[i][s - d][d][n];
					}
				}
			}
		}
...
bool converge_check(initial_parameter initial, basic_parameter basic, int iter) {
	...
	// allocation
	for (int i = 0; i < initial.nb_points; i++) {
		gap_check += abs(basic.x_val_B_old[i] - basic.x_val_B[i]) / (basic.x_val_B[i]+0.01);
		num_index++;
	}
	for (int i = 0; i < initial.nb_points; i++) {
		for (int t = 1; t <= initial.T; t++) {
			for (int n = 0; n < initial.nb_nodes; n++) {
				...
				for (int j = 0; j < initial.nb_points; j++) {
					if (j != i && basic.period[j][i] < t) {
						gap_check += abs(basic.flow_val_B_old[j][i][t - basic.period[j][i]][0][n] - basic.flow_val_B[j][i][t - basic.period[j][i]][0][n]) / (basic.flow_val_B[j][i][t - basic.period[j][i]][0][n]+0.01);
						gap_check += abs(basic.flow_val_B_old[j][i][t - basic.period[j][i]][1][n] - basic.flow_val_B[j][i][t - basic.period[j][i]][1][n]) / (basic.flow_val_B[j][i][t - basic.period[j][i]][1][n]+0.01);
						num_index += 2;
					}
				}
				// idle
				gap_check += abs(basic.flow_val_B_old[i][i][t - 1][2][n] - basic.flow_val_B[i][i][t - 1][2][n]) / (basic.flow_val_B[i][i][t - 1][2][n]+0.01);
				num_index++;
			}
		}
	}
	...
	for (int i = 0; i < initial.nb_points; i++) {
		for (int t = 1; t <= initial.T - 1; t++) {
			for (int s = 0; s < initial.nb_nodes; s++) {
				gap_check += abs(basic.flow_val_B_old[i][i][t][3][s] - basic.flow_val_B[i][i][t][3][s]) / (basic.flow_val_B[i][i][t][3][s]+0.01);
				num_index++;
			}
		}
	}
	...
	for (int n = 0; n < initial.nb_nodes; n++) {
		for (int i = 0; i < initial.nb_points; i++) {
			for (int s = 1; s <= initial.T; s++) {
				for (int d = 1; d <= 2; d++) {
					if (s - d >= 1) {
						gap_check += abs(basic.re_val_B_old[i][s - d][d][n] - basic.re_val_B[i][s - d][d][n])/ (basic.re_val_B[i][s - d][d][n]+0.01);
						num_index++;
					}
				}
			}
	...
	if (gap_check <= 0.03 * num_index || iter >= 100) {
		val_check = 0;
	}

	cout << "Gap = " << gap_check << endl << endl;
...
void start_point(initial_parameter initial, basic_parameter& basic, primal_variable primal) {
	...
	for (int n = 0; n < initial.nb_nodes; n++) {
		for (int k = 1; k < initial.nb_points; k++) {
			for (int s = 2; s <= initial.T; s++) {
				for (int d = 1; d <= 2; d++) {
					if (s - d >= 1) {
						basic.re_val_B[k][s - d][d][n] = 0;
					}
				}
			}
		}
	...
	for (int n = 1; n < initial.nb_nodes; n++) {
		for (int k = 1; k < initial.nb_points; k++) {
			for (int s = 2; s < initial.T; s++) {
				basic.flow_val_B[k][k][s][3][n] = 1;
			}
		}
	}
...
void output_nonlinear(initial_parameter initial, basic_parameter basic, primal_variable primal, string root) {
	...
	outresult << "Total" << endl;
	for (int i = 0; i < initial.nb_points; i++) {
		outresult << basic.x_val_A[i] << ',';
		sum_park_1 += basic.x_val_A[i];
	}
	outresult << sum_park_1 << ',' << ',';
	for (int i = 0; i < initial.nb_points; i++) {
		outresult << basic.x_val_B[i] << ',';
		sum_park_2 += basic.x_val_B[i];
	}
	outresult << sum_park_2 << endl;
	...
	for (int i = 0; i < initial.nb_points; i++) {
		for (int j = 0; j < initial.nb_points; j++) {
			for (int t = 1; t <= initial.T - 1; t++) {
				for (int s = 0; s < initial.nb_nodes; s++) {
					if (j != i && t + basic.period[i][j] <= initial.T) {
						satisfied_dmd_A.at(s) += basic.flow_val_A[i][j][t][0][s];
						satisfied_dmd_B.at(s) += basic.flow_val_B[i][j][t][0][s];
					}
				}
			}
		}
	...
	for (int i = 0; i < initial.nb_points; i++) {
		for (int ii = 0; ii < initial.nb_points; ii++) {
			for (int t = 1; t <= initial.T - 1; t++) {
				for (int s = 0; s < initial.nb_nodes; s++) {
					if (i != ii && t + basic.period[i][ii] <= initial.T) {
						dmd_A.at(s) += basic.dmd_val_A[i][ii][t][s];
						dmd_B.at(s) += basic.dmd_val_B[i][ii][t][s];
					}
				}
			}
		}
	...
	// -------- demand --------
	outresult << "Demand" << endl << "FirmA" << ',' << "FirmB" << ',' << "Total" << endl;
	outresult << total_dmd_A << ',' << total_dmd_B << ',' << total_dmd;

	// -------- transfer --------
	double transfer_A = 0; double transfer_B = 0; double total_transfer = 0;
	for (int i = 0; i < initial.nb_points; i++) {
		for (int t = 1; t <= initial.T - 1; t++) {
			for (int s = 0; s < initial.nb_nodes; s++) {
				transfer_A += basic.probTheta.at(s) * basic.flow_val_A[i][i][t][3][s];
				transfer_B += basic.probTheta.at(s) * basic.flow_val_B[i][i][t][3][s];
			}
		}
	}

	outresult << endl << "Trasnfer" << endl;
	outresult << "Firm1" << ',' << "Firm2" << ',' << "Total" << endl;
	outresult << transfer_A << ',' << transfer_B << ',' << transfer_A+transfer_B << endl;
}