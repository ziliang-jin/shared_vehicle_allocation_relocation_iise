	...
	*/
	...
	for (int k = 0; k < basic.nb_firms; k++) {
		for (int i = 0; i < initial.nb_points; i++) {
			for (int t = 0; t <= initial.T - 1; t++) {
				for (int s = 0; s < initial.nb_nodes; s++) {
					// idlecap_comp_cons.add(primal.flow[k][i][i][t][2][s] - primal.park[k][i] + bigm * dual.z_etaI[k][i][t][s] >= 0);
					// idlecap_comp_cons.add(primal.flow[k][i][i][t][2][s] - primal.park[k][i] - bigm * dual.z_etaI[k][i][t][s] <= 0);
					// idlecap_comp_cons.add(dual.etaI[k][i][t][s] + bigm * (1 - dual.z_etaI[k][i][t][s]) >= 0);
					// idlecap_comp_cons.add(dual.etaI[k][i][t][s] - bigm * (1 - dual.z_etaI[k][i][t][s]) <= 0);
				...
				for (int t = 1; t < initial.T; t++) {
					if (t + basic.period[i][ii] <= initial.T) {
						for (int s = 0; s < initial.nb_nodes; s++) {
							...
							// rentalvar_comp_cons.add(primal.flow[k][i][ii][t][0][s] - bigm * dual.z_dwR[k][i][ii][t][t + basic.period[i][ii]][s] <= 0);
							// rentalvar_comp_cons.add(dual.dwR[k][i][ii][t][t + basic.period[i][ii]][s] + bigm * (1 - dual.z_dwR[k][i][ii][t][t + basic.period[i][ii]][s]) >= 0);
							// rentalvar_comp_cons.add(dual.dwR[k][i][ii][t][t + basic.period[i][ii]][s] - bigm * (1 - dual.z_dwR[k][i][ii][t][t + basic.period[i][ii]][s]) <= 0);
...
void create_firmA_problem(initial_parameter initial, basic_parameter& basic, primal_variable primal) {
	...
	for (int j = 0; j < initial.nb_points; j++) {
		sumx += primal.park[0][j];
	}

	obj1 += basic.cost_purchase[0] * sumx;
	...
	for (int s = 0; s < initial.nb_nodes; s++) {
		...
		for (int i = 0; i < initial.nb_points; i++) {
			for (int ii = 0; ii < initial.nb_points; ii++) {
				for (int t = 1; t < initial.T; t++) {
					if (i != ii && t + basic.period[i][ii] <= initial.T) {
						sum_rental_cost += -basic.rental_price[0] * primal.flow[0][i][ii][t][0][s];
						sum_rental_cost += basic.penalty[0] * (primal.demandvar[0][i][ii][t][s] - primal.flow[0][i][ii][t][0][s]);
					}
				}
			}
		}
		...
		for (int i = 0; i < initial.nb_points; i++) {
			for (int ii = 0; ii < initial.nb_points; ii++) {
				for (int t = 1; t < initial.T; t++) {
					if (i != ii && t + basic.period[i][ii] <= initial.T) {
						sum_relocation_cost += basic.h[0][1] * primal.flow[0][i][ii][t][1][s];
					}
				}
			}
		}
		...
		for (int i = 0; i < initial.nb_points; i++) {
			for (int t = 0; t < initial.T; t++) {
				sum_idle_cost += basic.h[0][2] * primal.flow[0][i][i][t][2][s];
			}
		}
		//----- costs of transfer arcs -----
		IloExpr sum_trans_cost(basic.env);
		for (int i = 0; i < initial.nb_points; i++) {
			for (int t = 1; t <= initial.T - 1; t++) {
				sum_trans_cost += -basic.trans_price[0] * primal.flow[0][i][i][t][3][s];
				sum_trans_cost += basic.trans_price[1] * basic.flow_val_B[i][i][t][3][s];
			}
		}
		//----- total costs of stage 2 -----
		obj2 += basic.probTheta[s] * (sum_rental_cost + sum_relocation_cost + sum_idle_cost + sum_trans_cost);
	...
	totalcost = obj1 + obj2;
	basic.myobj = IloMinimize(basic.env, totalcost);

	basic.model.add(basic.myobj);
	...
	IloExpr totalallocationexpr(basic.env);
	for (int j = 0; j < initial.nb_points; j++) {
		totalallocationexpr += primal.park[0][j];
	}
	basic.mycons.add(totalallocationexpr - basic.budget[0] <= 0);
	totalallocationexpr.end();

	//----------- Allocation Cons -----------

	for (int j = 0; j < initial.nb_points; j++) {
		basic.mycons.add(primal.park[0][j] + basic.x_val_B[j] <= basic.totallots[j]);
	}
	...
	for (int i = 0; i < initial.nb_points; i++) {
		for (int t = 1; t <= initial.T; t++) {
			for (int ii = 0; ii < initial.nb_points; ii++) {
				for (int s = 0; s < initial.nb_nodes; s++) {
					if (i != ii && t + basic.period[i][ii] <= initial.T) {
						basic.mycons.add(primal.demandvar[0][i][ii][t][s] - basic.Y1[i][ii][t][t + basic.period[i][ii]][s]*primal.y[i][t][s] - basic.Y[0][i][ii][t][t + basic.period[i][ii]][s] == 0);
					}
				}
			}
		}
	...
	for (int i = 0; i < initial.nb_points; i++) {
		for (int t = 1; t <= initial.T; t++) {
			for (int n = 0; n < initial.nb_nodes; n++) {
				// sum in-flow
				IloExpr sumin_A_y(basic.env);
				IloExpr sumin_B_y(basic.env);
				IloExpr sumin_A(basic.env);
				// rental & relocation
				for (int j = 0; j < initial.nb_points; j++) {
					if (j != i && basic.period[j][i] < t) {
						for (int m = 0; m < basic.M; m++) {
							sumin_A_y += pow(2,m) * (primal.sigma[m][j][i][t - basic.period[j][i]][0][n] + primal.sigma[m][j][i][t - basic.period[j][i]][1][n]);
							sumin_A += pow(2, m) * (primal.z_binary[m][j][i][t - basic.period[j][i]][0][n] + primal.z_binary[m][j][i][t - basic.period[j][i]][1][n]);
						}
						sumin_B_y += (basic.flow_val_B[j][i][t - basic.period[j][i]][0][n] + basic.flow_val_B[j][i][t - basic.period[j][i]][1][n]) * primal.y[i][t - basic.period[j][i]][n];
					}
				}
				// idle
				for (int m = 0; m < basic.M; m++) {
					sumin_A_y += pow(2, m) * primal.sigma[m][i][i][t - 1][2][n];
					sumin_A += pow(2, m) * primal.z_binary[m][i][i][t - 1][2][n];
				}
				sumin_B_y += basic.flow_val_B[i][i][t - 1][2][n] * primal.y[i][t - 1][n];
				// return
				for (int d = 1; d <= 2; d++) {
					if (t - d >= 1) {
						sumin_A_y += basic.re_val_B[i][t - d][d][n] * primal.y[i][t - d][n];
						sumin_A += basic.re_val_B[i][t - d][d][n];
						for (int m = 0; m < basic.M; m++) {
							sumin_B_y += pow(2, m) * primal.sigma_re[m][i][t - d][d][n];
						}
					}
				}
				
				basic.mycons.add(sumin_A_y + sumin_B_y - sumin_A == 0);
	...
	for (int i = 0; i < initial.nb_points; i++) {
		for (int t = 1; t <= initial.T; t++) {
			for (int n = 0; n < initial.nb_nodes; n++) {
				...
				for (int j = 0; j < initial.nb_points; j++) {
					if (j != i && basic.period[j][i] < t) {
						IloExpr sum_rental_bi(basic.env);
						IloExpr sum_relo_bi(basic.env);

						for (int m = 0; m < basic.M; m++) {
							basic.mycons.add(primal.sigma[m][j][i][t - basic.period[j][i]][0][n] - primal.z_binary[m][j][i][t - basic.period[j][i]][0][n] <= 0);
							basic.mycons.add(primal.sigma[m][j][i][t - basic.period[j][i]][1][n] - primal.z_binary[m][j][i][t - basic.period[j][i]][1][n] <= 0);

							basic.mycons.add(primal.y[i][t - basic.period[j][i]][n] - primal.sigma[m][j][i][t - basic.period[j][i]][0][n] + primal.z_binary[m][j][i][t - basic.period[j][i]][0][n] <= 1);
							basic.mycons.add(primal.y[i][t - basic.period[j][i]][n] - primal.sigma[m][j][i][t - basic.period[j][i]][1][n] + primal.z_binary[m][j][i][t - basic.period[j][i]][1][n] <= 1);

							sum_rental_bi += pow(2, m) * primal.z_binary[m][j][i][t - basic.period[j][i]][0][n];
							sum_relo_bi += pow(2, m) * primal.z_binary[m][j][i][t - basic.period[j][i]][1][n];
						}
						basic.mycons.add(primal.flow[0][j][i][t - basic.period[j][i]][0][n] - sum_rental_bi == 0);
						basic.mycons.add(primal.flow[0][j][i][t - basic.period[j][i]][1][n] - sum_relo_bi == 0);
				...
				// idle
				IloExpr sum_idle_bi(basic.env);
				for (int m = 0; m < basic.M; m++) {
					basic.mycons.add(primal.sigma[m][i][i][t - 1][2][n] - primal.z_binary[m][i][i][t - 1][2][n] <= 0);
					basic.mycons.add(primal.y[i][t - 1][n] - primal.sigma[m][i][i][t - 1][2][n] + primal.z_binary[m][i][i][t - 1][2][n] <= 1);
					sum_idle_bi += pow(2, m) * primal.z_binary[m][i][i][t - 1][2][n];
				}
				basic.mycons.add(primal.flow[0][i][i][t - 1][2][n] - sum_idle_bi == 0);
				sum_idle_bi.end();
				// return
				for (int d = 1; d <= 2; d++) {
					if (t - d >= 1) {
						IloExpr sum_re_bi(basic.env);
						for (int m = 0; m < basic.M; m++) {
							basic.mycons.add(primal.sigma_re[m][i][t - d][d][n] - primal.z_re_binary[m][i][t - d][d][n] <= 0);
							basic.mycons.add(primal.y[i][t - d][n] - primal.sigma_re[m][i][t - d][d][n] + primal.z_re_binary[m][i][t - d][d][n] <= 1);

							sum_re_bi += pow(2, m) * primal.z_re_binary[m][i][t - d][d][n];
						}
						basic.mycons.add(primal.re[0][i][t - d][d][n] - sum_re_bi == 0);
						sum_re_bi.end();
					}
				}
	...
	for (int n = 0; n < initial.nb_nodes; n++) {
		for (int k = 0; k < initial.nb_points; k++) {
			for (int s = 1; s < initial.T; s++) {
				...
				for (int j = 0; j < initial.nb_points; j++) {
					if (j != k && basic.period[j][k] < s) {
						sumin += primal.flow[0][j][k][s - basic.period[j][k]][0][n] + primal.flow[0][j][k][s - basic.period[j][k]][1][n];
					}
				}
				// idle & transfer
				sumin += primal.flow[0][k][k][s - 1][2][n] + basic.flow_val_B[k][k][s][3][n];
				// return
				for (int d = 1; d <= 2; d++) {
					if (s - d >= 1) {
						sumin += basic.re_val_B[k][s - d][d][n];
					}
				}
				// sumout
				// rental & relocation
				for (int j = 0; j < initial.nb_points; j++) {
					if (j != k) {
						sumout += primal.flow[0][k][j][s][0][n] + primal.flow[0][k][j][s][1][n];
					}
				}
				// idle & transfer
				sumout += primal.flow[0][k][k][s][2][n] + primal.flow[0][k][k][s][3][n];
				// return
				for (int d = 1; d <= 2; d++) {
					if (s - d >= 1) {
						sumout += primal.re[0][k][s - d][d][n];
					}
				}

				basic.mycons.add(sumin - sumout == 0);
	...
	for (int n = 0; n < initial.nb_nodes; n++) {
		for (int j = 0; j < initial.nb_points; j++) {
			basic.mycons.add(primal.flow[0][j][j][0][2][n] - primal.park[0][j] == 0);
		}
	}
	// s=T
	for (int n = 0; n < initial.nb_nodes; n++) {
		for (int k = 0; k < initial.nb_points; k++) {
			...
			for (int j = 0; j < initial.nb_points; j++) {
				if (j != k && basic.period[j][k] < initial.T) {
					suminT += primal.flow[0][j][k][initial.T - basic.period[j][k]][0][n] + primal.flow[0][j][k][initial.T - basic.period[j][k]][1][n];
				}
			}
			// idle
			suminT += primal.flow[0][k][k][initial.T - 1][2][n];
			// return
			for (int d = 1; d <= 2; d++) {
				if (initial.T - d >= 1) {
					suminT += basic.re_val_B[k][initial.T - d][d][n];
				}
			}
			// sumoutT
			// return
			for (int d = 1; d <= 2; d++) {
				if (initial.T - d >= 1) {
					sumoutT += primal.re[0][k][initial.T - d][d][n];
				}
			}

			basic.mycons.add(suminT - sumoutT - primal.park[0][k] == 0);
	...
	for (int j = 0; j < initial.nb_points; j++) {
		for (int k = 0; k < initial.nb_points; k++) {
			for (int s = 1; s < initial.T; s++) {
				for (int n = 0; n < initial.nb_nodes; n++) {
					if (k != j && s + basic.period[j][k] <= initial.T) {
						basic.mycons.add(primal.flow[0][j][k][s][0][n] - primal.demandvar[0][j][k][s][n] <= 0);
					}
				}
			}
		}
	...
	for (int j = 0; j < initial.nb_points; j++) {
		for (int s = 1; s <= initial.T; s++) {
			for (int n = 0; n < initial.nb_nodes; n++) {
				basic.mycons.add(primal.flow[0][j][j][s - 1][2][n] - primal.park[0][j] <= 0);
			}
		}
	}
	//----------- Return Cons -----------
	if (basic.sharing_option == 1) {
		for (int t = 1; t <= initial.T - 1; t++) {
			for (int s = 0; s < initial.nb_nodes; s++) {
				double sumtransin = 0;
				for (int i = 0; i < initial.nb_points; i++) {
					sumtransin += basic.flow_val_B[i][i][t][3][s];
				}
				IloExpr sumreturn(basic.env);
				for (int i = 0; i < initial.nb_points; i++) {
					for (int d = 1; d <= std::min(2, initial.T - t); d++) {
						sumreturn += primal.re[0][i][t][d][s];
					}
				}
				basic.mycons.add(sumtransin - sumreturn == 0);
				sumreturn.end();
			}
		}
	}
...
void create_firmB_problem(initial_parameter initial, basic_parameter& basic, primal_variable primal) {
	...
	for (int j = 0; j < initial.nb_points; j++) {
		sumx += primal.park[1][j];
	}

	obj1 += basic.cost_purchase[1] * sumx;
	...
	for (int s = 0; s < initial.nb_nodes; s++) {
		...
		for (int i = 0; i < initial.nb_points; i++) {
			for (int ii = 0; ii < initial.nb_points; ii++) {
				for (int t = 1; t < initial.T; t++) {
					if (i != ii && t + basic.period[i][ii] <= initial.T) {
						sum_rental_cost += -basic.rental_price[1] * primal.flow[1][i][ii][t][0][s];
						sum_rental_cost += basic.penalty[1] * (primal.demandvar[1][i][ii][t][s] - primal.flow[1][i][ii][t][0][s]);
					}
				}
			}
		}
		...
		for (int i = 0; i < initial.nb_points; i++) {
			for (int ii = 0; ii < initial.nb_points; ii++) {
				for (int t = 1; t < initial.T; t++) {
					if (i != ii && t + basic.period[i][ii] <= initial.T) {
						sum_relocation_cost += basic.h[1][1] * primal.flow[1][i][ii][t][1][s];
					}
				}
			}
		}
		...
		for (int i = 0; i < initial.nb_points; i++) {
			for (int t = 0; t < initial.T; t++) {
				sum_idle_cost += basic.h[1][2] * primal.flow[1][i][i][t][2][s];
			}
		}
		//----- costs of transfer arcs -----
		IloExpr sum_trans_cost(basic.env);
		for (int i = 0; i < initial.nb_points; i++) {
			for (int t = 1; t <= initial.T - 1; t++) {
				sum_trans_cost += -basic.trans_price[1] * primal.flow[1][i][i][t][3][s];
				sum_trans_cost += basic.trans_price[0] * basic.flow_val_A[i][i][t][3][s];
			}
		}
		//----- total costs of stage 2 -----
		obj2 += basic.probTheta[s] * (sum_rental_cost + sum_relocation_cost + sum_idle_cost + sum_trans_cost);
	...
	IloExpr totalallocationexpr(basic.env);
	for (int j = 0; j < initial.nb_points; j++) {
		totalallocationexpr += primal.park[1][j];
	}
	basic.mycons.add(totalallocationexpr - basic.budget[1] <= 0);
	totalallocationexpr.end();
	//----------- Allocation Cons -----------
	for (int j = 0; j < initial.nb_points; j++) {
		basic.mycons.add(primal.park[1][j] + basic.x_val_A[j] <= basic.totallots[j]);
	}
	//========================== Second Stage ==========================
	...
	for (int i = 0; i < initial.nb_points; i++) {
		for (int t = 1; t <= initial.T; t++) {
			for (int ii = 0; ii < initial.nb_points; ii++) {
				for (int s = 0; s < initial.nb_nodes; s++) {
					if (i != ii && t + basic.period[i][ii] <= initial.T) {
						basic.mycons.add(primal.demandvar[1][i][ii][t][s] - basic.Y1[i][ii][t][t + basic.period[i][ii]][s] * primal.y[i][t][s]
							- basic.Y[1][i][ii][t][t + basic.period[i][ii]][s] == 0);
					}
				}
			}
		}
	...
	for (int i = 0; i < initial.nb_points; i++) {
		for (int t = 1; t <= initial.T; t++) {
			for (int n = 0; n < initial.nb_nodes; n++) {
				...
				IloExpr sumin_B_y(basic.env);
				IloExpr sumin_A_y(basic.env);
				IloExpr sumin_B(basic.env);
				// rental & relocation
				for (int j = 0; j < initial.nb_points; j++) {
					if (j != i && basic.period[j][i] < t) {
						for (int m = 0; m < basic.M; m++) {
							sumin_B_y += pow(2, m) * (primal.sigma[m][j][i][t - basic.period[j][i]][0][n] + primal.sigma[m][j][i][t - basic.period[j][i]][1][n]);
							sumin_B += pow(2, m) * (primal.z_binary[m][j][i][t - basic.period[j][i]][0][n] + primal.z_binary[m][j][i][t - basic.period[j][i]][1][n]);
						}
						sumin_A_y += (basic.flow_val_A[j][i][t - basic.period[j][i]][0][n] + basic.flow_val_A[j][i][t - basic.period[j][i]][1][n]) * primal.y[i][t - basic.period[j][i]][n];
					}
				}
				// idle
				for (int m = 0; m < basic.M; m++) {
					sumin_B_y += pow(2, m) * primal.sigma[m][i][i][t - 1][2][n];
					sumin_B += pow(2, m) * primal.z_binary[m][i][i][t - 1][2][n];
				}
				sumin_A_y += basic.flow_val_A[i][i][t - 1][2][n] * primal.y[i][t - 1][n];
				// return
				for (int d = 1; d <= 2; d++) {
					if (t - d >= 1) {
						sumin_B_y += basic.re_val_A[i][t - d][d][n] * primal.y[i][t - d][n];
						sumin_B += basic.re_val_A[i][t - d][d][n];
						for (int m = 0; m < basic.M; m++) {
							sumin_A_y += pow(2, m) * primal.sigma_re[m][i][t - d][d][n];
						}
					}
				}

				basic.mycons.add(sumin_B_y + sumin_A_y - sumin_B == 0);
			...
			for (int n = 0; n < initial.nb_nodes; n++) {
				...
				for (int j = 0; j < initial.nb_points; j++) {
					if (j != i && basic.period[j][i] < t) {
						...
						basic.mycons.add(primal.flow[1][j][i][t - basic.period[j][i]][0][n] - sum_rental_bi == 0);
						basic.mycons.add(primal.flow[1][j][i][t - basic.period[j][i]][1][n] - sum_relo_bi == 0);
				...
				basic.mycons.add(primal.flow[1][i][i][t - 1][2][n] - sum_idle_bi == 0);
				sum_idle_bi.end();
				// return
				for (int d = 1; d <= 2; d++) {
					if (t - d >= 1) {
						...
						basic.mycons.add(primal.re[1][i][t - d][d][n] - sum_re_bi == 0);
						sum_re_bi.end();
					}
				}
	...
	for (int n = 0; n < initial.nb_nodes; n++) {
		for (int k = 0; k < initial.nb_points; k++) {
			for (int s = 1; s < initial.T; s++) {
				...
				for (int j = 0; j < initial.nb_points; j++) {
					if (j != k && basic.period[j][k] < s) {
						sumin += primal.flow[1][j][k][s - basic.period[j][k]][0][n] + primal.flow[1][j][k][s - basic.period[j][k]][1][n];
					}
				}
				// idle & transfer
				sumin += primal.flow[1][k][k][s - 1][2][n] + basic.flow_val_A[k][k][s][3][n];
				// return
				for (int d = 1; d <= 2; d++) {
					if (s - d >= 1) {
						sumin += basic.re_val_A[k][s - d][d][n];
					}
				}
				// sumout
				// rental & relocation
				for (int j = 0; j < initial.nb_points; j++) {
					if (j != k) {
						sumout += primal.flow[1][k][j][s][0][n] + primal.flow[1][k][j][s][1][n];
					}
				}
				// idle & transfer
				sumout += primal.flow[1][k][k][s][2][n] + primal.flow[1][k][k][s][3][n];
				// return
				for (int d = 1; d <= 2; d++) {
					if (s - d >= 1) {
						sumout += primal.re[1][k][s - d][d][n];
					}
				}
	...
	for (int n = 0; n < initial.nb_nodes; n++) {
		for (int j = 0; j < initial.nb_points; j++) {
			basic.mycons.add(primal.flow[1][j][j][0][2][n] - primal.park[1][j] == 0);
		}
	}
	// s=T
	for (int n = 0; n < initial.nb_nodes; n++) {
		for (int k = 0; k < initial.nb_points; k++) {
			...
			for (int j = 0; j < initial.nb_points; j++) {
				if (j != k && basic.period[j][k] < initial.T) {
					suminT += primal.flow[1][j][k][initial.T - basic.period[j][k]][0][n] + primal.flow[1][j][k][initial.T - basic.period[j][k]][1][n];
				}
			}
			// idle
			suminT += primal.flow[1][k][k][initial.T - 1][2][n];
			// return
			for (int d = 1; d <= 2; d++) {
				if (initial.T - d >= 1) {
					suminT += basic.re_val_A[k][initial.T - d][d][n];
				}
			}
			// sumoutT
			// return
			for (int d = 1; d <= 2; d++) {
				if (initial.T - d >= 1) {
					sumoutT += primal.re[1][k][initial.T - d][d][n];
				}
			}

			basic.mycons.add(suminT - sumoutT - primal.park[1][k] == 0);
	...
	for (int j = 0; j < initial.nb_points; j++) {
		for (int k = 0; k < initial.nb_points; k++) {
			for (int s = 1; s < initial.T; s++) {
				for (int n = 0; n < initial.nb_nodes; n++) {
					if (k != j && s + basic.period[j][k] <= initial.T) {
						basic.mycons.add(primal.flow[1][j][k][s][0][n] - primal.demandvar[1][j][k][s][n] <= 0);
					}
				}
			}
		}
	...
	for (int j = 0; j < initial.nb_points; j++) {
		for (int s = 1; s <= initial.T; s++) {
			for (int n = 0; n < initial.nb_nodes; n++) {
				basic.mycons.add(primal.flow[1][j][j][s - 1][2][n] - primal.park[1][j] <= 0);
			}
		}
	}
	//----------- Return Cons -----------
	if (basic.sharing_option == 1) {
		...
		for (int t = 1; t <= initial.T - 1; t++) {
			for (int s = 0; s < initial.nb_nodes; s++) {
				double sumtransin = 0;
				for (int i = 0; i < initial.nb_points; i++) {
					sumtransin += basic.flow_val_A[i][i][t][3][s];
				}
				IloExpr sumreturn(basic.env);
				for (int i = 0; i < initial.nb_points; i++) {
					for (int d = 1; d <= std::min(2, initial.T - t); d++) {
						sumreturn += primal.re[1][i][t][d][s];
					}
				}
